...
class PlotGraph extends Component{
  ...
  render(){
    return(
      <Chart>
        <Axis count={10}
              labels={[{"index": 5, "label": this.getSuiteListLength()/2}, {"index": 10, "label": this.getSuiteListLength()}]}
              vertical={true} />
        <Chart vertical={true}>
          <Base height='medium'
                width='medium' />
          ...
          <Axis count={2}
                labels={[{"index": 0, "label": this.state.start}, {"index": 1, "label": this.state.end}]} />
        </Chart>
      </Chart>
    )
  }
